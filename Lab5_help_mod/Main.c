...
//debug code
int main(void){ 
	DisableInterrupts();						// bus clock at 50 MHz
	PortF_Init();
  LEDS = 0;													// turn all LEDs off
	DAC_Init(0x1000);
	SysTick_Init();
  Timer0A_Init(&Song_PlayHandler, Frequency_16HZ);		// initialize timer0A (16 Hz)
  EnableInterrupts();
  ...
  while(1){
		switch(Switches_In()) {
		  case 1  : TogglePlayPause(); break;	// If PE0 pressed, play C note  
			case 2  : RewindCurrentSong(); break;    // If PE1 pressed, play D note
			case 4  : TIMER0_TAILR_R = Frequency_64HZ-1; break;    // If PE2 pressed, play E note
			case 8  : TIMER0_TAILR_R = Frequency_16HZ-1; break;    // If PE3 pressed, play G note
		}
	}
}
