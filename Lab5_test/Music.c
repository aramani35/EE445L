...
uint32_t noteSongIndex = 0;
uint32_t beatSongIndex = 0;
...
Note* Song_CurrentNote() {
	return &currentPlayingSong.notes[noteSongIndex];
}

void Song_PlayHandler(void){
	Note currentNote = *Song_CurrentNote();
	beatSongIndex += 1;
	if(beatSongIndex >= currentNote.duration * 2) {
		noteSongIndex += 1;
		beatSongIndex = 0;
	}
}
...
void Rewind(void) {
	Pause();
	Song_PlayInit(currentPlayingSong);
}

unsigned long Sound_Period;
