...
uint32_t time_since_new_note = 0;
Note *prev_note = 0;
void SysTick_Handler(void) {
	// PF2 ^= 0x04;
	
	if(index_of_sound > 200) // 200 cyccle delay, makes notes separated
		DAC_Output(Sound_Config(index_of_sound, time_since_new_note));
	Note *newNote = Song_CurrentNote();
	if(newNote != prev_note) {
		prev_note = newNote;
		if(newNote->pitch == 0) // end of song
			NVIC_ST_RELOAD_R = 0;
		else
			NVIC_ST_RELOAD_R = newNote->pitch;
		index_of_sound %= 64;
		time_since_new_note = 0;
	}
	index_of_sound += 1;
	time_since_new_note += 1;
	// PF2 ^= 0x04;
}
